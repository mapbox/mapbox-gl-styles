...
export const styles = Object.fromEntries(styleIds.map((id) => {
    const file = id.split('.')[1] ?? id;
    return [id, readJSON(`styles/${file}.json`)];
}));

export const sprites = Object.fromEntries(spriteStyles
    .filter(style => style.includes('v8') || style.includes('v9'))
    .map((style) => {
        const svgDir = path.join(dir, 'sprites', style, '_svg');
        const entries = fs.readdirSync(svgDir)
            .filter(f => f.endsWith('.svg'))
            .map(f => ({
                svg: fs.readFileSync(path.join(svgDir, f)),
                id: f.replace('.svg', '')
            }));
